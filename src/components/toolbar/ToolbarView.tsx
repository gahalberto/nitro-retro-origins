    ...
    (event) => {
      const animationIconToToolbar = (
        ...
        if (!Motions.getMotionByTag(motionName)) {
          Motions.runMotion(
            new Queue(new Wait(wait + 8), new DropBounce(target, 400, 12))
          ).tag = motionName;
        }
      ...
      animationIconToToolbar("icon-bolsa", event.image, event.x, event.y);
    }
  );
        ...
        <Flex gap={2} alignItems="center">
          <Flex alignItems="center" gap={2}>
            <Flex
              ...
              pointer
              className={
                "navigation-item item-avatar " + (isMeExpanded ? "active " : "")
              }
          ...
          <Flex gap={2}>
            ...
            {GetConfiguration("game.center.enabled") && (
              <Base
                ...
                onClick={(event) => CreateLinkEvent("games/toggle")}
              />
            )}
            <Base
              pointer
            ...
            <Base
              pointer
              className="navigation-item icon icon-bolsa"
              onClick={(event) => CreateLinkEvent("bolsa/toggle")}
            >
              {getFullCount > 0 && <LayoutItemCountView count={getFullCount} />}
            </Base>
            {/* {isInRoom && (
            ...
            {(iconState === MessengerIconState.SHOW ||
              ...
              <Base
                ...
                className={`navigation-item icon icon-message ${
                  iconState === MessengerIconState.UNREAD && "is-unseen"
                }`}
